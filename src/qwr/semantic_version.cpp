...
std::optional<SemVer> SemVer::ParseString( const std::string& strVer )
{
    ...
    for ( const auto& splitView: qwr::string::Split( curScope, '.' ) )
    {
        const auto numRet = qwr::string::GetNumber<int8_t>( splitView );
        if ( !numRet )
        {
            return std::nullopt;
        }
...
bool SemVer::IsPreleaseNewer( std::string_view a, std::string_view b )
{
    ...
    while ( !a.empty() && !b.empty() )
    {
        ...
        if ( a_Token != b_Token )
        {
            ...
            {
                auto numRet = qwr::string::GetNumber<int8_t>( a_Token );
                assert( numRet ); // should be valid, because of `isNumber` check
                const int8_t aNum = *numRet;

                numRet = qwr::string::GetNumber<int8_t>( a_Token );
                assert( numRet ); // should be valid, because of `isNumber` check
